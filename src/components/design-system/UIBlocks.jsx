...
import { ArrowUp, Check, ChevronDown, Clock, Globe, Monitor, Moon, Paperclip, Square, Sun, Video, X } from 'lucide-react'
import { SpecimenSection } from './SpecimenSection.jsx'
import { SpecimenCard } from './SpecimenCard.jsx'
import '../../styles/ui-blocks.css'

function BlockSelect({ label, value, options, onChange }) {
  ...
  return <div className="v2-menu-select" ref={root} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false) }}>
    <button ref={trigger} className="v2-select-trigger v2-interactive-control" type="button" aria-label={`${label}: ${value}`} aria-haspopup="listbox" aria-expanded={open} aria-controls={id} onClick={() => setOpen(!open)} onKeyDown={(event) => {
  ...
  return <form className="v2-block v2-prompt-block" aria-label="Prompt composer" onSubmit={send}>
    ...
    <div className="v2-block-composer" data-dragging={dragging} onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setDragging(false) }} onDrop={(event) => { event.preventDefault(); setDragging(false); attach(event.dataTransfer.files) }}>
      ...
      <div className="v2-block-toolbar">
        ...
        <BlockSelect label="Prompt model" value={model} options={['Studio', 'Creative', 'Precise']} onChange={setModel} />
        <BlockSelect label="Prompt effort" value={effort} options={['Quick', 'Balanced', 'Thorough']} onChange={setEffort} />
...
export function SettingsBlock() {
  ...
  return <form className="v2-block v2-settings-block" aria-label="Workspace preferences" onSubmit={(event) => { event.preventDefault(); setSaved({...draft}); setNotice('Preferences saved for this preview.') }}>
    ...
    <div className="v2-preference-row"><div><strong>Language</strong><p>Default language for your workspace.</p></div><BlockSelect label="Workspace language" value={draft.language} options={['English', 'Deutsch', 'Français', '日本語']} onChange={(value) => update('language', value)} /></div>
...
export function UIBlocks() {
  return <SpecimenSection index={10} title="UI blocks" description="Complete patterns composed from our shared controls. Try each block; changes stay in this preview." className="v2-section--blocks">
    <SpecimenCard title="Prompt input" description="Attachments, commands, and a focused composer."><PromptInputBlock /></SpecimenCard>
