          ...
          <Field label="Campaign status" htmlFor="v2-campaign-status-trigger">
            <div className="v2-menu-select">
              <button
                ...
                id="v2-campaign-status-trigger"
                ref={statusTriggerRef}
                type="button"
                aria-haspopup="listbox"
                aria-expanded={isStatusOpen}
                aria-controls="v2-campaign-status-options"
                aria-label="Open campaign status options"
              ...
              {isStatusOpen && (
                <div
                  className="v2-floating-listbox"
                  id="v2-campaign-status-options"
                  role="listbox"
                  aria-label="Campaign status options"
                  onKeyDown={(event) => {
                    if (event.key === 'Escape') {
                      setIsStatusOpen(false)
                      statusTriggerRef.current?.focus()
                ...
                >
                  {campaignStatuses.map((status) => (
                    <button
                      ...
                      type="button"
                      role="option"
                      aria-selected={campaignStatus === status}
                      onClick={() => chooseStatus(status)}
                    >
                      {status}
                      {campaignStatus === status && <Check aria-hidden="true" size={18} />}
                    </button>
                  ))}
                </div>
              )}
