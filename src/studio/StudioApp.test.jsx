...
describe('connected studio workflow', () => {
  ...
  test('designer can inspect campaign brief but cannot edit or create campaigns', async () => {
    ...
    expect(screen.getByLabelText('Product or service')).toBeDisabled()
  ...
  test('retains a new campaign form when the server rejects creation', async () => {
    ...
    render(<ConnectedStudio api={api} />)
    await screen.findByRole('heading', { name: 'What are we creating?' })
    for (const [label, value] of [['Campaign name', 'My new campaign'], ['Product or service', 'New product'], ['Audience', 'Design teams'], ['Goal', 'Start trial']]) {
      fireEvent.change(screen.getByLabelText(label), { target: { value } })
    }
    fireEvent.click(screen.getByRole('button', { name: 'Create campaign' }))
    await screen.findByText('Campaign could not be saved')
    expect(screen.getByLabelText('Campaign name')).toHaveValue('My new campaign')
    expect(screen.getByLabelText('Audience')).toHaveValue('Design teams')
  ...
  test('retains unsaved edits after a stale revision save error', async () => {
    ...
    render(<ConnectedStudio api={api} />)
    await screen.findByRole('heading', { name: 'The campaign brief' })
    fireEvent.change(screen.getByLabelText('Campaign name'), { target: { value: 'Edited campaign' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save brief' }))
    await screen.findByText('Reload before saving')
    expect(api.patchCampaign).toHaveBeenCalledWith('campaign-1', { title: 'Edited campaign', brief }, 8)
    expect(screen.getByLabelText('Campaign name')).toHaveValue('Edited campaign')
    expect(screen.getByRole('button', { name: 'Save brief' })).toBeEnabled()
    expect(screen.getByRole('button', { name: 'Generate copy' })).toBeDisabled()
  })
  ...
  test.each(['pending', 'unknown'])('blocks another generation attempt while a persisted job is %s', async status => {
    ...
    render(<ConnectedStudio api={api} />)
    await screen.findByRole('heading', { name: 'The campaign brief' })
    const generate = screen.queryByRole('button', { name: 'Generate copy' })
    if (generate) {
  ...
  test('reuses the command identity after a transport error instead of making a duplicate paid request', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: 'Generate copy' })).toBeEnabled())
