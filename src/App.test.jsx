...
describe('Lingu Studio app', () => {
  ...
  test('waits for the designer store update, then records marketer approval before Delivery', async () => {
    ...
    const downloadAssets = screen.getByRole('button', { name: 'Download assets' })
    expect(downloadAssets).toBeVisible()
    expect(downloadAssets.closest('footer')).toHaveClass('stage-actions--top')
    expect(downloadAssets.compareDocumentPosition(screen.getAllByRole('article', { name: 'Template preview Split frame' })[0])).toBe(Node.DOCUMENT_POSITION_FOLLOWING)
    ...
    vi.stubGlobal('URL', { createObjectURL, revokeObjectURL })
    vi.stubGlobal('Blob', BlobMock)
    await user.click(downloadAssets)
    expect(createObjectURL).toHaveBeenCalledTimes(1)
