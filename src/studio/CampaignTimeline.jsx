...
export function CampaignTimeline({ workspace, stage, pending, onChange }) {
  ...
  return <aside className="bs-campaign-timeline" data-expanded={expanded} aria-label="Campaign progress">
    ...
    <div id="campaign-timeline-steps">
      <StepRail items={items} hiddenSteps={[]} currentStep={stage + 1} maxStep={currentStage(workspace) + 1} disabled={Boolean(pending)} scrollOnChange={false} ariaLabel="Campaign workflow" className="bs-timeline-rail" onStepChange={number => { onChange(number - 1); setExpanded(false) }} />
    </div>
  </aside>
}
