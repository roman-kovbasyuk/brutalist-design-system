...
export const sectionEntries = { 1: 'Application tokens', 9: 'Catalog specimens', 10: 'PromptInputBlock, SchedulingBlock, SettingsBlock', 11: 'PromptComposer' }
...
export function PreviewMetadata({ name, title, description, compact = false }) {
  ...
  return <header className="ds-preview-metadata" id={previewId(name)} tabIndex={-1}>
    {!compact && <h3>{title || entry.name}</h3>}
