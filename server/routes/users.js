import { z } from 'zod'
import { createInvitationRequestSchema } from '../../shared/contracts.js'
import { parse, requireActor } from './support.js'
...
export function registerUserRoutes(app, { resolveActor, workflowService }) {
  app.post('/api/v1/invitations', async (request, reply) => {
    ...
    reply.code(201)
    return workflowService.createInvitation({ actor, input })
  })
  ...
  app.post('/api/v1/users/:userId/disable', async (request) => {
    ...
    return workflowService.disableUser({ actor, userId })
  })
}
