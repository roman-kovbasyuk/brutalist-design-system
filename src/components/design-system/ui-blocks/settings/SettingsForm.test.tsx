...
test('groups settings accessibly and restores the saved appearance after cancel', async () => {
  ...
  expect(within(screen.getByRole('group', { name: 'Workspace' })).getByLabelText('Workspace name')).toHaveValue('Studio')
  expect(within(screen.getByRole('group', { name: 'Notifications' })).getByRole('switch', { name: 'Mentions' })).toBeChecked()
...
test('uses standard panels for each workspace settings group', () => {
  ...
  expect(screen.getByRole('group', { name: 'Workspace' })).toHaveClass('ds-settings-form__section')
})
